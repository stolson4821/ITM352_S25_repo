...
# Function to load the list of questions from a JSON file
# Opens the file containing the questions and loads it as a JSON object
def load_questions():
    with open("a1questions.json", "r") as file:
        return json.load(file)

# Function to load existing high scores from the scores file
# Tries to open the scores file and load the data if it exists
# If the file is not found, return an empty dictionary indicating no scores
def load_scores():
    try:
...
# Function to save the current scores to the scores file
...
# Asks the user to input their username and removes any extra spaces
def get_user():
...
def ask_question(question_data):
    ...
    # Return the score for this question (0 points if incorrect on both attempts)
    return score
